    ...
    try:
        ...
        if sessions:
            for row in sessions:
                ...
                label = f"{risk_icon} {row['pr_url'].split('/')[-1]} — {row['last_event'][:16]}"
                if st.button(label, key=row["thread_id"], use_container_width=True):
...
def render_escalation_card(payload: dict) -> dict | None:
    ...
    if submitted_esc and all(answers.values()):
        return answers
    return None
