    ...
    with console.status("[dim]LLM reviewing the diff...[/dim]"):
        analysis = llm.invoke([
            {"role": "system", "content": (
                ...
                "with 2–4 specific, context-rich questions referencing which file/section in the diff you are uncertain about."
            )},
...
def node_synthesize(state: ReviewState) -> dict:
    ...
    with console.status("[dim]LLM refining review with reviewer answers...[/dim]"):
        refined = llm.invoke([
            {"role": "system", "content": "Refine the code review using the reviewer's answers. Structured output."},
            {"role": "user", "content": (
                f"Original diff:\n{state['pr_diff']}\n\n"
